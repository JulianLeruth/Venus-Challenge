...
#define ROBOT_WIDTH 12.3                            // Measured distance between the middle of the wheels of the robot.
#define NUMBER_OF_DIST_MEASUREMENT_PER_SECOND 20    // The set number of measurements the distance sensor can take.

#define SMALL_BLOCK_MIN_NUM_MEASUREMENTS 9          // The minimum number of measurements it should take for the robot to detect a small block.
#define SMALL_BLOCK_MAX_NUM_MEASUREMENTS 14         // The maximum number of measurements it can take for the robot to detect a small block and the minimum for the robot to detect a big block.
#define BIG_BLOCK_MAX_NUM_MEASUREMENTS 20           // The maximum number of measurements it can take for the robot to detect a big block.
...
int highestValueIndex(int* arr, int size) {
    int index = 0;
...
int lowestValueIndex(int* arr, int size) {
    int index = 0;
    for (int i = 1; i < size; i++) {
        if (arr[0] == 0) {
            index = 1;
...
void straigth(int dist /* Defined in cm */, uint16_t speed /* Defined in ??? ~ Max is 65535 & Min is 3024 */){
    stepper_reset();
    stepper_enable();
...
void turnRight(int angle /* Defined in degrees */, uint16_t speed /* Defined in ??? ~ Max is 65535 & Min is 3024 */){
...
void turnLeft(int angle /* Defined in degrees */, uint16_t speed /* Defined in ??? ~ Max is 65535 & Min is 3024 */){
    float number_of_steps = (ROBOT_WIDTH*2*PI*angle*STRAIGTH_STEPS_IN_CM)/360;
...
void twist(int angle /* Defined in degrees */, uint16_t speed /* Defined in ??? ~ Max is 65535 & Min is 3024 */){
...
void straveRight(int angle, int dist /* Defined in degrees */, uint16_t speed /* Defined in ??? ~ Max is 65535 & Min is 3024 */){
...
void straveLeft(int angle, int dist /* Defined in degrees */, uint16_t speed /* Defined in ??? ~ Max is 65535 & Min is 3024 */){
...
void dance(void) {
    ...
    twist(180, 15000);
    waitTillDone();
    straveLeft(30,10, 15000);
    waitTillDone();
    for(int i = 0; i < 5; i++){
        straveRight(-60,10, 15000);
        waitTillDone();
        straveLeft(60,10, 15000);
        waitTillDone();
    }
    straveRight(-30,10, 15000);
    waitTillDone();
}
...
void objectDectection(vl53x sensor) {
    ...
    int lowest_distance_index = lowestValueIndex(iDistanceArr, NUMBER_OF_DIST_MEASUREMENT_PER_SECOND);
    ...
    // printf("]\n");
    twist(lowest_distance_index*360/(NUMBER_OF_DIST_MEASUREMENT_PER_SECOND*speed), speed);
    waitTillDone();
}